...
public class puesto extends javax.swing.JFrame {
    ...
    private void cargarListado() {
        ArrayList<String[]> datosPuestos = leerdatosPuestosDesdeArchivo();

        String[] columnas = {"ID", "Descripcion"};
        tableModel = new DefaultTableModel(columnas, 0);

        for (String[] fila : datosPuestos) {
    ...
    private ArrayList<String[]> leerdatosPuestosDesdeArchivo() {
        ArrayList<String[]> datosPuestos = new ArrayList<>();
    ...
    private void tablaUsuariosKeyPressed(java.awt.event.KeyEvent evt) {                                         
    ...
    private void formularioTabsMouseClicked(java.awt.event.MouseEvent evt) {                                            
    ...
    // ID del puesto que deseas eliminar (reemplaza con el valor correcto)
    int postIdToDelete = Integer.parseInt(deptoid.getText());

    try {
        File file = new File(filePath);
        Scanner scanner = new Scanner(file);
        StringBuilder content = new StringBuilder();
        boolean postsSection = false;

        while (scanner.hasNextLine()) {
            String line = scanner.nextLine().trim();

            if (line.trim().equals("- PUESTO")) {
                postsSection = true;
            } else if (postsSection && !line.trim().isEmpty() && !line.startsWith("-")) {
                ...
                if (fields.length >= 1) {
                    try {
                        ...
                        if (postId == postIdToDelete) {
                            ...
                            // Si el ID del puesto coincide con el que queremos eliminar, no agregamos la línea al contenido
                            JOptionPane.showMessageDialog(null, "Puesto eliminado correctamente");
                            limpiarCampos();
                            continue;
                        }
                    } catch (NumberFormatException e) {
                        System.err.println("Error al convertir el ID de puesto: " + e.getMessage());
                    }
                }
            }
        ...
        System.out.println("Puesto con ID " + postIdToDelete + " eliminado correctamente.");
    } catch (IOException e) {
    ...
    private void btnEditActionPerformed(java.awt.event.ActionEvent evt) {                                        
       String filePath = archivo_patch;
        ...
        int postIdToUpdate = Integer.parseInt(deptoid.getText());
        ...
        try {
                ...
                if (line.trim().equals("- PUESTO")) {
                    ...
                    if (fields.length >= 1 && fields[0].matches("\\d+")) {
                        ...
                        if (postId == postIdToUpdate) {
                            content.append(postIdToUpdate + "|" + newdeptodesc).append("\n");
                            continue;
                        }
                    }
                    content.append(line).append("\n");
            ...
            System.out.println("Puesto con ID " + postIdToUpdate + " actualizado correctamente.");
        } catch (IOException e) {
            System.err.println("Error al leer o escribir en el archivo: " + e.getMessage());
        }
    }                                       

    private void deptoidActionPerformed(java.awt.event.ActionEvent evt) {                                        
        // TODO add your handling code here:
    }                                       

    private void btnSaveActionPerformed(java.awt.event.ActionEvent evt) {                                        

    String postIdInput = deptoid.getText();
    if (!postIdInput.matches("[0-9]+")) {
        JOptionPane.showMessageDialog(null, "El ID debe ser numérico");
        return;
    }
    
    String postDescInput = deptodesc.getText();
    if (postDescInput.length() > 30) {
    ...
    try {
        ...
        // Iterar sobre el archivo para buscar el ID del puesto en la sección "PUESTO"
        while (scanner.hasNextLine()) {
            String line = scanner.nextLine().trim();
            if (line.equals("- PUESTO")) {
                // Entramos en la sección de puestos, comenzamos a buscar el ID
                while (scanner.hasNextLine()) {
                    String postLine = scanner.nextLine().trim();
                    if (postLine.startsWith(postIdInput + "|")) {
                        // Si se encuentra un ID igual, mostrar mensaje y salir del bucle
                        postExists = true;
                        break;
                    } else if (postLine.startsWith("-")) {
                        // Hemos alcanzado el final de la sección de puestos
                        break;
                    }
                }
        ...
        // Si el puesto ya existe, mostrar mensaje y salir del método
        if (postExists) {
            JOptionPane.showMessageDialog(null, "Ya existe un puesto con el mismo ID.");
            return;
        }

        // Construir la nueva entrada del puesto
        String newPostEntry = postIdInput + "|" + postDescInput + "\n";
        ...
        while (scanner.hasNextLine()) {
            ...
            if (line.trim().equals("- PUESTO")) {
                ...
                // Insertar el nuevo registro después de la sección de puestos
                content.append(newPostEntry);
                postsSection = false; // Evitar duplicar la inserción
            }
        }
        ...
        System.out.println("Nuevo puesto con ID " + postIdInput + " ha sido agregado a la sección Puestos en 'database.txt'.");
    ...
    private void deptodescActionPerformed(java.awt.event.ActionEvent evt) {                                          
        // TODO add your handling code here:
    }                                         
